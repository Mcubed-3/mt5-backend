...
CORS(
    ...
    allow_headers=["Content-Type", "X-API-Key", "X-Admin-Token", "X-Support-Token"],
...
SUPPORT_TOKEN = os.getenv("SUPPORT_TOKEN", "").strip()

# Optional IP allowlists (comma-separated). If empty -> no IP restriction.
ADMIN_ALLOWED_IPS = [x.strip() for x in os.getenv("ADMIN_ALLOWED_IPS", "").split(",") if x.strip()]
SUPPORT_ALLOWED_IPS = [x.strip() for x in os.getenv("SUPPORT_ALLOWED_IPS", "").split(",") if x.strip()]

EA_ONLINE_SECONDS = int(os.getenv("EA_ONLINE_SECONDS", "300"))  # 5 minutes default
...
def _client_ip():
    # Render / proxies: X-Forwarded-For may contain a list
...
def require_admin():
    if not ADMIN_TOKEN:
        return False, json_error("ADMIN_TOKEN not configured", 500)
    token = request.headers.get("X-Admin-Token", "").strip()
    if not token or token != ADMIN_TOKEN:
        return False, json_error("Unauthorized", 401)
    if not _ip_allowed(ADMIN_ALLOWED_IPS):
...
def require_support():
    if not SUPPORT_TOKEN:
        return False, json_error("SUPPORT_TOKEN not configured", 500)
    token = request.headers.get("X-Support-Token", "").strip()
    if not token or token != SUPPORT_TOKEN:
        return False, json_error("Unauthorized", 401)
    if not _ip_allowed(SUPPORT_ALLOWED_IPS):
        return False, json_error("Forbidden (IP not allowed)", 403)
    return True, None
...
def ea_connected(user: User) -> bool:
    ...
    last = _utc(user.last_seen_at)
    return (now - last).total_seconds() <= EA_ONLINE_SECONDS
...
def ensure_can_trade(user: User):
    # allow if paid active OR trial active
...
def ensure_schema():
    ...
    with db.engine.begin() as conn:
        ...
        def add(sql): conn.execute(text(sql))

        # base auth
        if "email" not in cols:
            add('ALTER TABLE "user" ADD COLUMN email VARCHAR(255)')
            add('CREATE UNIQUE INDEX IF NOT EXISTS ix_user_email ON "user"(email)')
        if "password_hash" not in cols:
            add('ALTER TABLE "user" ADD COLUMN password_hash VARCHAR(255)')
        if "api_key" not in cols:
            add('ALTER TABLE "user" ADD COLUMN api_key VARCHAR(64)')
            add('CREATE UNIQUE INDEX IF NOT EXISTS ix_user_api_key ON "user"(api_key)')

        # settings
        if "enabled" not in cols:
            add('ALTER TABLE "user" ADD COLUMN enabled BOOLEAN NOT NULL DEFAULT FALSE')
        if "pair" not in cols:
            add('ALTER TABLE "user" ADD COLUMN pair VARCHAR(20) NOT NULL DEFAULT \'XAUUSD\'')
        if "pairs" not in cols:
            add('ALTER TABLE "user" ADD COLUMN pairs VARCHAR(255) NOT NULL DEFAULT \'XAUUSD\'')
        if "lot_size" not in cols:
            add('ALTER TABLE "user" ADD COLUMN lot_size DOUBLE PRECISION NOT NULL DEFAULT 0.01')

        # timestamps/profile
        if "created_at" not in cols:
            add('ALTER TABLE "user" ADD COLUMN created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()')
        if "last_login_at" not in cols:
            add('ALTER TABLE "user" ADD COLUMN last_login_at TIMESTAMPTZ NULL')
        if "last_settings_at" not in cols:
            add('ALTER TABLE "user" ADD COLUMN last_settings_at TIMESTAMPTZ NULL')
        if "display_name" not in cols:
            add('ALTER TABLE "user" ADD COLUMN display_name VARCHAR(80) NULL')

        # sl/tp
        if "sl_mode" not in cols:
            add('ALTER TABLE "user" ADD COLUMN sl_mode VARCHAR(20) NOT NULL DEFAULT \'dynamic\'')
        if "tp_mode" not in cols:
            add('ALTER TABLE "user" ADD COLUMN tp_mode VARCHAR(20) NOT NULL DEFAULT \'rr\'')
        if "min_pips" not in cols:
            add('ALTER TABLE "user" ADD COLUMN min_pips INTEGER NOT NULL DEFAULT 50')
        if "sl_buffer_pips" not in cols:
            add('ALTER TABLE "user" ADD COLUMN sl_buffer_pips INTEGER NOT NULL DEFAULT 5')
        if "rr" not in cols:
            add('ALTER TABLE "user" ADD COLUMN rr DOUBLE PRECISION NOT NULL DEFAULT 1.0')
        if "pattern_tp_mult" not in cols:
            add('ALTER TABLE "user" ADD COLUMN pattern_tp_mult DOUBLE PRECISION NOT NULL DEFAULT 1.5')
        if "fixed_sl_pips" not in cols:
            add('ALTER TABLE "user" ADD COLUMN fixed_sl_pips INTEGER NOT NULL DEFAULT 50')
        if "fixed_tp_pips" not in cols:
            add('ALTER TABLE "user" ADD COLUMN fixed_tp_pips INTEGER NOT NULL DEFAULT 50')

        # billing
        if "plan" not in cols:
            add('ALTER TABLE "user" ADD COLUMN plan VARCHAR(20) NOT NULL DEFAULT \'free\'')
        if "subscription_status" not in cols:
            add('ALTER TABLE "user" ADD COLUMN subscription_status VARCHAR(30) NOT NULL DEFAULT \'none\'')
        if "trial_ends_at" not in cols:
            add('ALTER TABLE "user" ADD COLUMN trial_ends_at TIMESTAMPTZ NULL')
        if "stripe_customer_id" not in cols:
            add('ALTER TABLE "user" ADD COLUMN stripe_customer_id VARCHAR(80) NULL')
            add('CREATE INDEX IF NOT EXISTS ix_user_stripe_customer_id ON "user"(stripe_customer_id)')
        if "stripe_subscription_id" not in cols:
            add('ALTER TABLE "user" ADD COLUMN stripe_subscription_id VARCHAR(80) NULL')
            add('CREATE INDEX IF NOT EXISTS ix_user_stripe_subscription_id ON "user"(stripe_subscription_id)')

        # heartbeat
        if "last_seen_at" not in cols:
            add('ALTER TABLE "user" ADD COLUMN last_seen_at TIMESTAMPTZ NULL')
        if "last_seen_symbol" not in cols:
            add('ALTER TABLE "user" ADD COLUMN last_seen_symbol VARCHAR(20) NULL')
        if "last_seen_tf" not in cols:
            add('ALTER TABLE "user" ADD COLUMN last_seen_tf VARCHAR(10) NULL')
        if "last_seen_ip" not in cols:
            add('ALTER TABLE "user" ADD COLUMN last_seen_ip VARCHAR(64) NULL')

        # risk ack
        if "risk_ack_at" not in cols:
            add('ALTER TABLE "user" ADD COLUMN risk_ack_at TIMESTAMPTZ NULL')
...
# -------------------------
# Routes: Account/Profile (dashboard uses X-API-Key)
# -------------------------
@app.get("/account/me")
...
def account_me():
    ...
    return jsonify({
        ...
        "risk_ack_at": user.risk_ack_at.isoformat() if user.risk_ack_at else None,
    })
...
@app.post("/account/risk-ack")
@limiter.limit("30 per minute")
def account_risk_ack():
    """
    Optional: dashboard can call this when user ticks the risk checkbox.
    Not required for trading in backend (your UI gates Start Algo).
    """
    user, err = require_api_key()
    if err:
        return err

    user.risk_ack_at = datetime.now(timezone.utc)
    db.session.commit()
    return jsonify({"ok": True, "risk_ack_at": user.risk_ack_at.isoformat()})
...
# -------------------------
# EA Heartbeat (EA calls every N minutes)
# -------------------------
...
def heartbeat():
    ...
    user.last_seen_ip = _client_ip()

    db.session.commit()
    return jsonify({"ok": True, "last_seen_at": user.last_seen_at.isoformat()})
...
# -------------------------
# Admin: read/write routes (locked by ADMIN_TOKEN + optional IP allowlist)
# -------------------------
@app.get("/admin/users")
@limiter.limit("120 per minute")
def admin_users():
...
def admin_user_activity(user_id: int):
    ...
    return jsonify({
        ...
        "user": {
            ...
            "sl_mode": u.sl_mode,
            "tp_mode": u.tp_mode,
            "min_pips": int(u.min_pips),
            "sl_buffer_pips": int(u.sl_buffer_pips),
            "rr": float(u.rr),
            "pattern_tp_mult": float(u.pattern_tp_mult),
            "fixed_sl_pips": int(u.fixed_sl_pips),
            "fixed_tp_pips": int(u.fixed_tp_pips),
            ...
            "risk_ack_at": u.risk_ack_at.isoformat() if u.risk_ack_at else None,
        },
        "trades": [
            {
                "id": t.id,
...
# -------------------------
# Support: READ-ONLY routes (locked by SUPPORT_TOKEN + optional IP allowlist)
# -------------------------
@app.get("/support/users")
@limiter.limit("120 per minute")
def support_users():
    ok, err = require_support()
    if err:
        return err
    ...
    # NOTE: no api keys, no IPs, no billing IDs
    return jsonify({
        "ok": True,
        "items": [
            {
...
@app.get("/support/user/<int:user_id>/activity")
@limiter.limit("120 per minute")
def support_user_activity(user_id: int):
    ok, err = require_support()
    if err:
        return err
